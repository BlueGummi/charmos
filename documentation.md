...
Similar to the **style guide**, this is a documentation *guide*, not a set of documentation *rules*. There may be instances where documentation may deviate from this guide, and that is expected and allowed, but try to follow these guidelines with most things.
...
The **Idea Structure** defines "**Ideas**" of different "**Sizes**". As the **Size** of an **Idea** increases, the *scope* that it covers increases, but the *granularity* and *specificity* of the **Idea** decreases.
...
ALL **Huge Ideas** should go in the separate `./docs/` directory for documentation. These files should have a `.md` extension and be written in standard markdown format. The names should be structured as such:
...
To adhere to the style guide, all file names for **Huge Ideas** in the `/docs/` directory should be lowercase `snake_case`.
...
Pseudocode in **Ideas** is permitted, but don't be bureaucratic about the format of pseudocode. The following are examples of pseudocode that is acceptable:
...
**Small Ideas** should be the smallest **Ideas**, whereas **Huge Ideas** should be the second smallest, and **Big Ideas** should be the largest in length.
...
Diagrams are useful, but do not overuse them. Diagrams can be used to describe the parts of a large structure and interactions between them, but only when necessary to show connections between parts.
 ...
 *
 * Interactions: Locking is used almost everywhere that requires shared ownership...
 *
 ...
 *
 * References: [1] Big Idea "Turnstiles" in `./kernel/sync/turnstile.c`
 *
 * Changelog:
 ...
 *
 * References: [1] Huge Idea "Locking Philosophy" in `./docs/locking_idea.md`
 *
 * Changelog:
...
```c
/*
 * [1]: <Size> Idea "Name" in `./relative/path/to/idea/from/project/root`
 */
```
...
**Huge Ideas** should be more focused on *theory* and *interaction*, less on *implementation*. **Huge Ideas** serve to "paint a picture", not write a paper. **Huge Ideas** should be *shorter* than **Big Ideas**.

### Section 2: "**The Big Idea**"

**Big Ideas** are meant to be less abstract than **Huge Ideas**. Whereas **Huge Ideas** typically discuss the design philosophy of a subsystem or component, **Big Ideas** should focus on the implementations and private interactions of components of a **Huge Idea**.
...
**Big Ideas** should seek to thoroughly describe a component, and it is not unexpected for a **Big Idea** to be so thorough that **Small Ideas** are not necessary. However, **Big Ideas** can reference **Small Ideas** beneath them.
...
**Small Ideas** are internal. They are not for the outside world to see and read, and thus, many **Sections** of other **Ideas** are not present.
...
Typically, the smaller the **Idea**, the more frequently it should be updated. **Small Ideas** should be be updated whenever the functionality of the function or a set of functions it encompasses changes. **Big Ideas** should be changed whenever the component they describe has a major API change or a major internal detail change, and **Huge Ideas** should only be changed if there is a complete overhaul in design philosophy.
