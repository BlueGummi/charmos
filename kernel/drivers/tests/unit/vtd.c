...
TEST_DECLARE_UNIT(vtd_sl_iova_tiling, .group = TEST_GROUP(vtd_unit),
                  TEST_INTENSITY(1, 16, 4096)) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        TEST_ASSERT(pml4 < SL_ENTRY_COUNT);
        TEST_ASSERT(pdpt < SL_ENTRY_COUNT);
        TEST_ASSERT(pd < SL_ENTRY_COUNT);
        TEST_ASSERT(pt < SL_ENTRY_COUNT);
        ...
        TEST_ASSERT(reconstructed == iova);
    }
