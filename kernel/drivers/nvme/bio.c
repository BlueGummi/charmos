...
bool nvme_should_coalesce(struct generic_disk *disk, struct bio_request *a,
...
void nvme_add_prp_segment(struct nvme_bio_data *prp_data, const void *buffer,
                          uint64_t size) {
...
void nvme_do_coalesce(struct generic_disk *disk, struct bio_request *into,
    ...
    if (!into->driver_private2) {
        ...
        nvme_add_prp_segment(dd, into->buffer, into->size);
    }

    struct nvme_bio_data *dd = into->driver_private2;

    nvme_add_prp_segment(dd, from->buffer, from->size);
