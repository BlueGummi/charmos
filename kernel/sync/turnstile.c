...
struct turnstile *turnstile_init(struct turnstile *ts) {
    ...
    ts->inheritor = NULL;
    thread_queue_init(&ts->queues[TURNSTILE_READER_QUEUE]);
    thread_queue_init(&ts->queues[TURNSTILE_WRITER_QUEUE]);
    return ts;
}
...
static size_t turnstile_thread_priority(struct thread *t) {
...
/* we have exited the turnstile since we have woken up from block */
void turnstile_exit(struct turnstile *ts) {
    ...
    if (ts->waiters == 0) {
        ...
        /* take the top one on the freelist */
        kassert(!list_empty(&ts->freelist));
...
void turnstile_block(struct turnstile *ts, size_t queue_num, void *lock_obj,
                     enum irql lock_irql) {
    ...
    thread_block_on(&ts->queues[queue_num]);
}
