...
static struct vas_range *vasrange_freelist = NULL;

static void vasrange_refill() {
    ...
    for (uint64_t i = 0; i < VASRANGE_PER_PAGE; i++) {
        ranges[i].next_free = vasrange_freelist;
        vasrange_freelist = &ranges[i];
    }
}

struct vas_range *vasrange_alloc() {
    if (!vasrange_freelist)
        vasrange_refill();

    struct vas_range *r = vasrange_freelist;
    vasrange_freelist = r->next_free;
    return r;
}

void vasrange_free(struct vas_range *r) {
    r->next_free = vasrange_freelist;
    vasrange_freelist = r;
}
...
vaddr_t vas_alloc(struct vas_space *vas, size_t size, size_t align) {
    ...
    while (node) {
        ...
        if (prev_end + size <= vr->start) {
            struct vas_range *new_range = vasrange_alloc();
