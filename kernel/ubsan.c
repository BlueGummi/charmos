...
void __ubsan_handle_add_overflow(data_location_type_t *data, uintptr_t lhs,
                                 uintptr_t rhs) {
    printf("UBSAN: add_overflow @ %s:%u:%u {lhs: %#lx, rhs: %#lx, type: %u-bit "
           "%s %s}\n",
           data->location.filename, data->location.line, data->location.column,
...
void __ubsan_handle_sub_overflow(data_location_type_t *data, uintptr_t lhs,
                                 uintptr_t rhs) {
    printf("UBSAN: sub_overflow @ %s:%u:%u {lhs: %#lx, rhs: %#lx, type: %u-bit "
           "%s %s}\n",
           data->location.filename, data->location.line, data->location.column,
           lhs, rhs, info_to_bits(data->type->info),
           kind_to_type(data->type->kind), data->type->name);
...
void __ubsan_handle_mul_overflow(data_location_type_t *data, uintptr_t lhs,
                                 uintptr_t rhs) {
    printf("UBSAN: mul_overflow @ %s:%u:%u {lhs: %#lx, rhs: %#lx, type: %u-bit "
           "%s %s}\n",
           data->location.filename, data->location.line, data->location.column,
           lhs, rhs, info_to_bits(data->type->info),
           kind_to_type(data->type->kind), data->type->name);
...
void __ubsan_handle_type_mismatch(data_type_mismatch_t *data, void *pointer) {
    ...
    if (pointer == NULL) {
        printf("UBSAN: type_mismatch @ %s:%u:%u (%s NULL pointer of type %s)\n",
               data->location.filename, data->location.line,
               data->location.column, kind_strs[data->type_check_kind],
               data->type->name);
    } else if (data->alignment &&
               !is_aligned((uint64_t) pointer, data->alignment)) {
        printf("UBSAN: type_mismatch @ %s:%u:%u (%s misaligned address %#lx of "
               "type %s)\n",
               data->location.filename, data->location.line,
               data->location.column, kind_strs[data->type_check_kind],
               (uintptr_t) pointer, data->type->name);
    } else {
        printf(
